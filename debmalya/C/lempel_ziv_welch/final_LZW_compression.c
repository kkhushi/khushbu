...
int chk_dict(char *wc_ptr){    // Directory Checking function
 printf("chk = %s", wc_ptr);
int i=0,a=1000,b=0;
char wc_1[6]={0};
strcpy(wc_1,wc_ptr);
...
int i=256, count =256;
	...
	while (dctnry[i].code!=NULL){    // finding the last element in dictionary
	count++;i++;
	 }
...
FILE *fp;
int count=0,inp_len=0, j=0, if_in_dict=10, pagla=0;
