...
#    device = self.dev
    ...
    while lock and not option.ambush:
        lock = raw_input('Do you really want to copy the image file %s on %s?' 
                % (args[0], device_name))
        if lock in ('y', 'ye', 'yes'):
            lock = False
    ...
    if not option.dummy:
        ...
        exit_status = os.system('./livecd-iso-to-disk --noverify %s %s' 
                % (args[0], device_name))
        ...
        # Rerun l-i-t-d with --reset-mbr option
            ...
            exit_status = os.system('./livecd-iso-to-disk --noverify '
                    '--reset-mbr %s %s' % (args[0], device_name))
        ...
        # erase flage is here to point this out.
            ...
            parent_obj = bus.get_object('org.freedesktop.Hal', 
                    self.device.GetProperty('info.parent'))
            device_name = (dbus.Interface(parent_obj, 
                'org.freedesktop.Hal.Device')).GetProperty('block.device')
            os.system('/sbin/sfdisk %s << EOF\n,,6,*\n;\nEOF' % device_name)
            os.system('/sbin/mkdosfs -I -n usbdisk %s' % device_name + '1')
            exit_status = os.system('./livecd-iso-to-disk --noverify %s %s' 
                    % (args[0], device_name + '1'))
        elif exit_status == 0:
...
def check_device(udi):
    ...
    # unwanted automounting procedures 
        ...
        if (device.GetProperty('storage.bus') == 'usb' and 
            ...
            device.AcquireInterfaceLock('org.freedesktop.Hal.Device.Storage',
                    True)
#           device.Lock('KoboldLock')
	    return device
#           background = install_image(device)
#           background.start()
# 	    background.join()
#           device.Unlock()
#            device.ReleaseInterfaceLock
    ...
    # process and after that unlock the device
        ...
        parent_obj = bus.get_object('org.freedesktop.Hal', 
                device.GetProperty('info.parent'))
        ...
        if (parent.PropertyExists('info.locked') and 
            ...
            not parent.IsLockedByOthers('org.freedesktop.Hal.Device.Storage') 
            and parent.GetProperty('info.locked.reason') == 'KoboldLock'):

	    return device
#            back2 = install_image(device)
#	    back2.start()
#	    back2.join()
#            parent.Unlock()
...
if __name__ == '__main__':
    ...
    parser = OptionParser(usage='%prog [options] ISOFILE',
            version='%prog 0.1')
...
#    print len(device)
    ...
    thread = []
    while(len(device)):
      thread.append(install_image(device.pop()))
